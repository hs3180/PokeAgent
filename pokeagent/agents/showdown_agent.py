...
class ShowdownAgent(BaseAgent):
    ...
    def choose_move(self, battle):
        ...
        if battle.available_moves:
            ...
            return self.create_order(
                random.choice(
                    list(battle.available_moves) + list(battle.available_switches)
                )
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not auth_url:
            auth_url = websocket_url.replace("wss://", "https://").replace(
        ...
        server_config = ServerConfiguration(
            websocket_url=websocket_url, authentication_url=auth_url
        )
    ...
    async def join_ladder(self, battle_format: Optional[str] = None, n_games: int = 1):
        ...
        try:
            ...
            logging.info(
                f"已开始搜索 {format_to_use} 天梯对战，计划进行 {n_games} 场对战"
            )
        except Exception as e:
